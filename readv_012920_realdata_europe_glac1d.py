...
size = Line2D(range(4), range(4), color="black", marker='o', linewidth=0, linestyle='none',
              markersize=16, markerfacecolor="lightsalmon")
...
if ice_model == 'glac1d_':
    def build_dataset(model):
        ...
        modelrun = [key.split('glac1d/output_', 1)[1][:-3].replace('.','_') for key in basefiles]
    ...
    ds_sliced = ds.rsl.sel(age=slice(tmax, tmin), lon=slice(df_place.lon.min() - 2, df_place.lon.max() + 2),
              lat=slice(df_place.lat.min() - 2, df_place.lat.max() + 2))
    ...
    ds_diff = one_mod([ice_model+'l71C']).sel(modelrun=ice_model+lithmantle).rsl.sel(age=slice(tmax, tmin), 
                         ...
                         lat=slice(df_place.lat.min() - 2,df_place.lat.max() + 2)).load().to_dataset().interp(age=ds_readv.age, 
                                                                                                          lon=ds_readv.lon,
                                                                                                         lat=ds_readv.lat)
...
class GPR_diag(gpf.models.GPModel):
    ...
    def log_likelihood(self):
        ...
        jitter = tf.cast(tf.fill([num_data], default_jitter()), 'float64') # stabilize K matrix w/jitter
...
print('negative log marginal likelihood =', m.neg_log_marginal_likelihood().numpy())
...
da_giapriorinterp.to_netcdf(path + ice_model + lith_thickness + '_' + place +'_giaprior')
da_priorplusgpr.to_netcdf(path + ice_model + lith_thickness + '_' + place +'_posterior')
da_varp.to_netcdf(path + ice_model + lith_thickness + '_' + place +'_gp_variance')
...
for i, age in enumerate(ages):
    if (age/500).is_integer():
        ...
        pc1 = ds_giapriorinterp.rsl[i].transpose().plot(ax=ax1, transform=proj, 
                                                        cmap=cmap, 
                                                        norm=MidpointNormalize(vmin, vmax, 0),
                                                        add_colorbar=False, extend='both'
                                                       )
        cbar = fig.colorbar(pc1, ax=ax1, 
                            shrink=.3, 
                            label='RSL (m)',
                            extend='both')
        scat = ax1.scatter(lon_it, lat_it,
                           s=80, 
                           c=rsl, 
                           edgecolor='k',
                           vmin=vmin, 
...
#         ax3.set_extent(extent_)
        ...
        pc = (2 * np.sqrt(da_varp[i])).plot(ax=ax4, transform=proj,
                                      vmin=vmin_std, 
                                      vmax=vmax_std * 2, 
                                      cmap='Reds',  
                                      extend='both',
        ...
        scat = ax4.scatter(lon_it, lat_it, s=80, c= 2 * np.sqrt(var), vmin=vmin_std, vmax=vmax_std * 2,
...
for i, site in enumerate(df_nufsamps.groupby('locnum')):
    ...
    site_err = 2*(site[1].rsl_er_max + site[1].rsl_giaprior_std)
    
    ax[i].scatter(site[1].age, site[1].rsl, c=colors[0], label='"true" RSL')
    ax[i].errorbar(site[1].age, site[1].rsl, site_err, c=colors[0], fmt='none', 
                   capsize=1, lw=1,)
...
fig.savefig(dirName + f'{ages[0]}to{ages[-1]}_{place}_realdata_fig_1D', transparent=True)
...
plt.tight_layout()
fig.savefig(dirName + f'{ages[0]}to{ages[-1]}_{place}_realdata_fig_1Dlocs', transparent=True)
...
A3, var3 = reshape_decomp(k3, var=df_place.rsl_er_max.ravel()**2 + df_place.rsl_giaprior_std.ravel()**2) #readvance spatial
A4, var4 = reshape_decomp(k4, var=df_place.rsl_er_max.ravel()**2 + df_place.rsl_giaprior_std.ravel()**2) #readvance temporal
A5, var5 = reshape_decomp(k5, var=df_place.rsl_er_max.ravel()**2 + df_place.rsl_giaprior_std.ravel()**2) #readvance spatial
# A6, var6 = reshape_decomp(k6, var=df_place.rsl_er_max.ravel()**2 + df_place.rsl_giaprior_std.ravel()**2) #readvance temporal
...
fig.savefig(dirName + f'{ages[0]}to{ages[-1]}_{place}_decompkernels', transparent=True)
